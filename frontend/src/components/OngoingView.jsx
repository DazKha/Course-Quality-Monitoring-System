...
function OngoingView() {
  ...
  const [criticalFilter, setCriticalFilter] = useState('all'); // 'all', 'Phase 1', 'Phase 2', 'Phase 3'
  ...
  const getLatestPrediction = (courseData) => {
    // Check from Phase 3 -> Phase 2 -> Phase 1
    for (let i = courseData.length - 1; i >= 0; i--) {
  ...
  // Only count courses that are CURRENTLY at that stage (not courses that have progressed beyond)
  const getStageStats = () => {
    const stages = ['Phase 1', 'Phase 2', 'Phase 3'];
    return stages.map(stageName => {
      // Filter courses where current stage equals this stage
      const coursesAtThisStage = courses.filter(c => getCurrentStage(c.data) === stageName);

      // Get predictions for courses at this stage
      const predictions = coursesAtThisStage.map(c => {
        const stageData = c.data.find(d => d.stage === stageName);
        return stageData?.prediction;
      }).filter(p => p); // Remove null/undefined

      const needsImprovement = predictions.filter(p => p === 'Needs Improvement').length;
      const acceptable = predictions.filter(p => p === 'Acceptable').length;
      const excellent = predictions.filter(p => p === 'Excellent').length;
      const total = coursesAtThisStage.length;
      const notReached = courses.length - courses.filter(c => {
        // Count courses that have reached this stage or beyond
        const currentStage = getCurrentStage(c.data);
      ...
      <div className="space-y-4">
        ...
        <div className="bg-slate-800 rounded-lg p-6 border border-slate-700">
          ...
          <div className="mt-4 text-slate-400 text-sm space-y-2">
            <p>📊 Biểu đồ thể hiện phân bố dự đoán tại mỗi giai đoạn. Số lượng khóa học giảm dần vì chưa tất cả đều đạt Phase 2, Phase 3.</p>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${criticalFilter === 'all'
                ...
                {['Phase 1', 'Phase 2', 'Phase 3'].map(stage => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${criticalFilter === stage
                        ? 'bg-red-600 text-white shadow-lg'
                        : 'bg-slate-700 text-slate-300 hover:bg-slate-600'
                        }`}
                    >
            ...
            <table className="w-full">
              <thead className="bg-slate-900 sticky top-0">
                <tr>
                  ...
                  <th className="px-4 py-3 text-center text-xs font-medium text-slate-400 uppercase">Phase 1</th>
                  <th className="px-4 py-3 text-center text-xs font-medium text-slate-400 uppercase">Phase 2</th>
                  <th className="px-4 py-3 text-center text-xs font-medium text-slate-400 uppercase">Phase 3</th>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {selectedCourse.data.map((stage, idx) => (
                    <div
                      key={idx}
                      className={`bg-slate-900 rounded-lg p-4 border-l-4 ${stage.prediction
